...
int main() {
    ...
    while (t--) {
        ...
        vector<vector<vector<pair<pair<int, int>, long long >>>> tunnel(n + 1,
                                                                        vector<vector<pair<pair<int, int>, long long >>>(
                                                                                m + 1));
        for (int i = 0; i < p; ++i) {
            cin >> x1 >> y1 >> x2 >> y2 >> w;
        ...
        while (!qq.empty()) {
            ...
            for (int i = 1; i <= 8; ++i) {
                ...
                if (xx > 0 && xx <= n && yy > 0 && yy <= m && mat[xx][yy][0] != -1 &&
                    mat[xx][yy][i] > mat[p.second.first][p.second.second][p.first] + costxx) {
                    mat[xx][yy][i] = mat[p.second.first][p.second.second][p.first] + costxx;
                    qq.push({i, {xx, yy}});
                }
            }
            ...
            for (auto &i : tunnel[p.second.first][p.second.second]) {
                int mn = 0;
                for (int k = 1; k < 9; ++k) {
                    if (mat[i.first.first][i.first.second][0] != -1 && mat[i.first.first][i.first.second][0] >
                                                                       mat[p.second.first][p.second.second][k] +i.second
                            ) {
                        mat[i.first.first][i.first.second][0] = mat[p.second.first][p.second.second][k] + i.second;
